...
/*   Updated: 2024/03/26 14:47:06 by rgramati         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ms_open_dquote(int tmp_file_fd, t_qstate qs)
{
	char	*line;
...
int	ms_get_dquote(char *line, t_envvar **env, char *tmp)
{
	...
	{
		rl_catch_signals = 1;
		ms_signal_state(SIGHANDLER_DQU);
		tmp_file_fd = open(tmp, OPEN_EXCL, 0600);
		ms_dq_holder((char *)&tmp_file_fd, 1);
		rl_clear_history();
		ms_clear_env(*env);
		free(tmp);
		write(tmp_file_fd, line, ft_strlen(line));
		ms_quote_enforcer(&line, tmp_file_fd, QU_ZERO);
		free(line);
...
t_error	ms_quote_handler(char **line, t_envvar **envp, int status)
{
	...
	if (!*line)
		return (ERR_ERRORS);
	history_line = *line;
	dquote_file = ms_get_temp_file(".dquote", 16);
	if (ms_quote_error(*line, NULL, QU_ZERO))
	{
		status = ms_get_dquote(*line, envp, dquote_file);
		history_line = ms_get_dquote_line(*line, dquote_file, status);
	}
	...
	*line = history_line;
	...
	if (WEXITSTATUS(status) == 130)
		return (ERR_FAILED);
	qs = ft_strdup(" ");
	if (ms_quote_error(*line, qs, QU_ZERO))
		ms_error_message(ERR_DQSTOP, qs);
	free(qs);
	return (ERR_NOERRS);
}
