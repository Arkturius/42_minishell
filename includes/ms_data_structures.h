...
#ifndef MS_DATA_STRUCTURES_H
# define MS_DATA_STRUCTURES_H
 ...
 */
t_token		*ms_init_token(char *str, t_token_type type);
 ...
 */
void		ms_add_token(t_token **token, t_token *next);
 ...
 */
t_envvar	*ms_init_var(char *value);
 ...
 */
t_command	*ms_init_command(t_redir *redirs, char **args, t_envvar **envp);
 ...
 */
t_node		*ms_init_node(t_command *command, t_token *token);
 ...
 */
void		ms_associate(t_node **tree, t_node *next, t_command *c, t_token *t);
...
*/
void		ms_pipes_push(t_pipes **head, t_pipes *top);
...
*/
t_pipes		*ms_pipes_pop(t_pipes **head);
